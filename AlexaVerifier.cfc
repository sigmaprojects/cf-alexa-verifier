component output=false {
	...
	variables.TIMESTAMP_TOLERANCE	= 1500000000000000000000000;
	variables.VALID_CERT_HOSTNAME	= 's3.amazonaws.com';
	variables.VALID_CERT_PATH_START	= '/echo.api/';
	variables.VALID_CERT_PORT		= '443';
	variables.VALID_CERT_SAN		= 'echo-api.amazon.com';
	variables.LOG_REQUESTS			= true;
	variables.LOG_REQUESTS_DIR		= expandPath('./logs/');

	/** 
	* Validates all requirements.  Stops processing once a failure occurs.
	* This does not accept implicit arguments, but the proper headers and body content is expected:
	* @SignatureCertChainUrl Header: String URL to the certificate
	* @Signature Header: String signature
	* @content Body: Normal json
	*/
	remote struct function verifier() returnFormat='json' {
		var result = {
			success = true,
			errors = [],
			data = ''
		...
		try {
			...
			if( !structKeyExists(requestData,'content') || !len(trim(requestData.content)) ) {
	...
	/** 
	* Validates all requirements.  Stops processing once a failure occurs.
	* @signatureCertChainUrlHeader: String URL to the certificate
	* @signatureHeader: String signature
	* @contentBody: Normal json
	*/
	public struct function verifierManual(String signatureCertChainUrlHeader='', String signatureHeader='', String contentBody='') {
		var result = {
			success = true,
			errors = [],
			data = ''
	...
	/** 
	* Validates the timestamp in the request
	* @requestBody The json body as a String
	*/
	remote struct function validateTimestamp(Required String requestBody) returnFormat='json' {
		var result = {
			success = true,
			data = '',
			errors = []
	...
	/** 
	* Verifies the request body against the certificate and signature. 
	* BE WARNED: If passing a string to requestBody via serializeJSON(cfObject) ACF may change "true/false" to "yes/no", and other characters
	* may be uppercase.  This will cause this method to fail verification.
	* @pem_cert The Certificate
	* @signatureBase64 The request Signature
	* @requestBody The JSON Request Body as a String
	*/
	remote struct function isValidSignature(Required String pem_cert, Required Any signatureBase64, String requestBody='') returnFormat='json' {
		var result = {
			success = false,
			errors = [],
			data = ''
	...
	* @cert_uri The URL to the certificate
	*/
	remote struct function validateCertUri(cert_uri) returnFormat='json' {
		var result = {
			success = true,
			data = '',
			errors = []
	...
	/** 
	* Validates the certificate, making sure it's not expired and the correct SANs exist.
	* @pem_cert The Certificate as a String
	*/
	remote struct function validateCert(String pem_cert='') returnFormat='json' {
		var result = {
			success = true,
			errors = [],
			data = ''
	...
	/** 
	* Makes a HTTP call to download the certificate
	* @href URL to the certificate
	*/
	remote struct function fetchCert(Required String href) returnFormat='json' {
		...
		if( returnObject.success ) {
			if( isSimpleValue(local.result.Filecontent) ) {
				...
				returnObject.data = toString(local.result.Filecontent);
			}
		}
	...
	/** 
	* Transforms a given String to a of java Certificate
	* @pem_string Certificate as String
	*/
	private any function generateCertificateFromString(Required String pem_string) {
